...
> **レビュアーとして起動された場合、このスキルには従わないこと。** これは依頼する側の手順であり、レビュアー向けの指示ではない。上の「レビュアーには別のプロセスを使い」は、レビュアーを立てる側の行動を指す。レビュアーがこれを自分宛と読むと、さらに別のエージェントへ委譲する連鎖が起きる。依頼されたレビューは自分で行う。
...
Herdr 管理下（`test "${HERDR_ENV:-}" = 1`）で外部 CLI が使えるなら、別 pane で起動する。`herdr` の構文は、`herdr` skill が入っていればそれに従う。無ければ `herdr --help` と各コマンドグループ（`herdr workspace` など、サブコマンドを省いて実行する）の出力で確認する。構文の権威はインストール済みのバイナリであり、版によって差がある。レビュアーに `claude` を選んだ場合も `launching-claude-remote-control` は使わない。同 skill は `claude --remote-control` 専用で、trust folder プロンプトの確定や `agent: claude`・`/remote-control is active` を成否判定に使う。レビューに remote control は要らず、codex や cursor-agent ではその判定自体が通らない。
...
- **a. レビュアー自身がレビューする。** さらに別のエージェントへ委譲させない。対象リポジトリに `third-party-review` が置かれていると、レビュアーがこれを自分宛の手順と読み、委譲の連鎖を始めることがある。依頼文で「あなたが最終レビュアー」「`third-party-review` には従わない」と明示する。CLI 自身のレビュー機能（Claude の `/code-review` など）は使ってよいが、「リポジトリ内のレビュー用 skill を探して使う」という促し方はしない。名前でマッチするのが `third-party-review` 自身になるため。
- **b. 指摘は GitHub の行コメントで書く。** PR 全体への総括コメント 1 つで済ませない。該当行に紐付けないと、どの記述への指摘か追えず、返信も resolve もスレッド単位で扱えなくなる。
  ...
  - review の `event` は `COMMENT` にする。レビュアーは依頼者の GitHub アカウントで API を叩くため、対象は自分の PR になる。自分の PR には `REQUEST_CHANGES` も `APPROVE` も付けられず、422 で拒否される（`REQUEST_CHANGES` なら `Review Can not request changes on your own pull request`）。
...
- CLI 自身のレビュー機能（`/code-review` など）は使って構いません
- 指摘は GitHub の行コメントとして該当行に付けてください。総括コメント 1 つにまとめないでください
  （diff の行に紐付けられない指摘だけは、総括コメントに書いて構いません）
- 複数の指摘は 1 つの review にまとめて submit してください。`event` は `COMMENT` にしてください
  （自分の PR には `REQUEST_CHANGES` を付けられず 422 になります）
...
| review を `REQUEST_CHANGES` で submit して 422 になる | 自分の PR には付けられない。`event` は `COMMENT` にさせる |
